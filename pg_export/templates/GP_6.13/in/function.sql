select json_agg(x)
  from (select quote_ident(n.nspname) as schema,
               ...
               ({% include 'GP_6.13/in/_argument.sql' %}) as arguments,
         ...
         inner join pg_namespace tn
          ...
          {% with objid='p.oid', objclass='pg_proc' -%} {% include 'GP_6.13/in/_join_description_as_d.sql' %} {% endwith %}
         where n.nspname not in ('pg_catalog', 'pg_toast', 'information_schema') and
               {% with objid='p.oid', objclass='pg_proc' %} {% include 'GP_6.13/in/_not_part_of_extension.sql' %} {% endwith %}
