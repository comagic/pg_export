select json_agg(x)
  from (select format_type(c.castsource, -1) as source,
          ...
          from pg_cast c
          {% with objid='c.oid', objclass='pg_cast' -%} {% include 'PG_12/in/_join_description_as_d.sql' %} {% endwith %}
         where c.oid > {{ last_builin_oid }} and
               {% with objid='c.oid', objclass='pg_cast' %} {% include 'PG_12/in/_not_part_of_extension.sql' %} {% endwith %}
         order by 1, 2) as x
