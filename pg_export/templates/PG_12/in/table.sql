select json_agg(x)
  from (select quote_ident(n.nspname) as schema,
                         ...
                         inner join pg_type ct
                          ...
                          left join pg_attrdef cd
                                 on cd.adrelid = a.attrelid and
                                    cd.adnum = a.attnum
                          {% with objid='c.oid', objclass='pg_class', objsubid='a.attnum' -%} {% include 'PG_12/in/_join_description_as_d.sql' %} {% endwith %}
               ...
               ({% include 'PG_12/in/_index.sql' %}) as indexes,
         ...
         where c.relkind in ('r', 'p', 'f') and
               ...
               {% with objid='c.oid', objclass='pg_class' %} {% include 'PG_12/in/_not_part_of_extension.sql' %} {% endwith %}
         order by 1, 2) as x
