select json_agg(x)
  from (select quote_ident(n.nspname) as schema,
         ...
         inner join pg_namespace n
          ...
          {% with objid='o.oid', objclass='pg_operator' -%} {% include '12/in/_join_description_as_d.sql' %} {% endwith %}
         where n.nspname not in ('pg_catalog', 'pg_toast', 'information_schema') and
               {% with objid='o.oid', objclass='pg_operator' %} {% include '12/in/_not_part_of_extension.sql' %} {% endwith %}
         order by 1, 2) as x
